...
echo "timm"
srun --partition=medai_p --mpi=pmi2 --gres=gpu:$NGPUS --quotatype=spot -n1 --ntasks-per-node=1 --cpus-per-task=32 --job-name=Comclip --kill-on-bad-exit=1 \
torchrun --nproc_per_node=8 --master_port=2222 ./pretrain/train.py \
    --train_csv_path ./data/Broad-270/compound_splits/train5.csv \
    --val_csv_path ./data/Broad-270/compound_splits/val5.csv \
    ...
    --save_dir ./pretrain_ckpt/setting1split2705 \
    --pretrained \
