...
def print_detailed_stats(epoch: int, step: int, train_loss: float, eval_loss: float, metrics: Dict[str, Any], prefix: str = "Val"):
...
def evaluate_distributed(model: nn.Module, loader: DataLoader, cfg: DSConfig, assay_embeds: torch.Tensor, desc: str = "Val") -> Dict[str, Any]:
    ...
    for batch in it:
        ...
        with autocast(device_type="cuda"):
            out = model(images, smiles, assay_embeds)
    ...
    if rank == 0:
        ...
        if len(ys) > 0:
            ...
            results = compute_metrics_standard(y_true_nan, y_prob)

            results["raw_probs"] = y_prob
            results["raw_targets"] = y_true_nan
            results["loss"] = global_loss
        else:
...
def run_eval_and_save(
    ...
    val_macro_auc = val_results["macro_auc"]
    ...
    if rank == 0:
        ...
        print_detailed_stats(epoch, global_step, current_train_loss, val_loss, val_results, prefix="Val")
        if test_results:
            print_detailed_stats(epoch, global_step, current_train_loss, test_results["loss"], test_results, prefix="Test")

        log_dict = {
            "epoch": epoch,
            "train/loss": current_train_loss,
            "val/loss": val_loss,
            "val/auroc_macro": val_macro_auc,
        }
        ...
        if val_macro_auc > best_metric:
            best_metric = val_macro_auc
            torch.save({
...
def train(cfg: DSConfig):
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        for batch in pbar:
            ...
            with autocast(device_type="cuda"):
                out = model(images, smiles, train_assay_embeds)
                loss = masked_bce_with_logits(out["logits"], targets, mask)
            ...
            if global_step % cfg.interval_step == 0:
                ...
                best_metric = run_eval_and_save(
                    cfg, model.module,
                    val_loader, val_assay_embeds, val_assay_ids,
                    test_loader, test_assay_embeds, test_assay_ids,
                    epoch, global_step, current_train_loss, best_metric, best_path
                )
...
def parse_args() -> DSConfig:
    ...
    p.add_argument("--petrel_conf_path", type=str, default=None)
